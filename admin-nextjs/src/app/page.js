                  ...
                  <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                      ...
                      {activeModal === 'warning' && (
                          <div className="space-y-4">
                              ...
                              <input value={formState.title} onChange={e=>setFormState({...formState, title: e.target.value})} placeholder="Incident Class" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg outline-none font-bold text-[11px] focus:border-indigo-500" />
                              <textarea rows={8} value={formState.message} onChange={e=>setFormState({...formState, message: e.target.value})} placeholder="Debrief report..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg outline-none font-medium text-[11px] resize-none focus:border-indigo-500" />
                          </div>
                      )}
                      {activeModal === 'wallet' && (
                      ...
                      {activeModal === 'broadcast' && (
                          <div className="space-y-4">
                              <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Target</label>
                              ...
                              <textarea rows={6} value={formState.message} onChange={e=>setFormState({...formState, message: e.target.value})} placeholder="Message body..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg outline-none font-medium text-[11px] resize-none" />
                          </div>
                      )}
                      {activeModal === 'details' && selectedUser && (
                          <div className="space-y-8">
                              ...
                              <div className="space-y-4">
                                  <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-[3px] border-b pb-2">Activity Pulse</h4>
    ...
    <div className="p-6 space-y-6 h-full flex flex-col font-sans">
        ...
        {selectedPartner && (
            <div className="bg-white p-6 rounded-[32px] shadow-2xl border border-slate-100 flex items-center justify-between gap-10 animate-in slide-in-from-bottom-2 duration-300">
                ...
                <div><p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 opacity-50">Coordination Link</p><p className="font-bold text-slate-800 text-xs tracking-widest font-mono">{selectedPartner.lat?.toFixed(5) || '0.00'}, {selectedPartner.lng?.toFixed(5) || '0.00'}</p><p className="text-[8px] font-bold text-emerald-500 mt-1 uppercase">Active Link Verified</p></div>
...
const AnalyticsView = ({ data }) => !data ? null : (
  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 animate-in fade-in duration-300">
    ...
    <div className="col-span-full bg-white p-10 rounded-[32px] border border-slate-200 shadow-sm mt-4">
        <h3 className="font-bold text-slate-800 uppercase tracking-[4px] text-[10px] mb-12 text-center opacity-40">Global Resource Matrix</h3>
...
const UsersTable = ({ users, onWarn, onWallet, onDelete, onBlock, onDetails, title }) => (
  <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden animate-in fade-in duration-300">
    <div className="px-8 py-5 border-b border-slate-100 bg-white flex justify-between items-center"><h3 className="font-bold text-slate-800 text-[11px] uppercase tracking-widest">{title} Registry</h3><span className="px-4 py-1 bg-slate-50 rounded-full text-[9px] font-bold text-slate-400 uppercase tracking-tighter">{users?.length || 0} Registered Units</span></div>
...
const PendingView = ({ providers, onApprove }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in duration-300">
    {providers.length === 0 ? (
       <div className="col-span-full p-40 bg-white rounded-3xl border-4 border-dashed border-slate-100 text-center text-slate-200 font-bold uppercase text-[10px] tracking-[8px]">Registry Nominal</div>
    ) : (
      providers.map(p => (
        <div key={p.uid} className="bg-white p-8 rounded-3xl border border-slate-200 relative group overflow-hidden hover:shadow-2xl transition-all text-center">
          ...
          <h4 className="font-bold text-sm tracking-tight text-slate-900 uppercase truncate px-4">{p.name}</h4>
...
const WithdrawalsTable = ({ withdrawals, onHandle }) => (
  <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden animate-in fade-in duration-300">
    <div className="px-10 py-6 border-b border-slate-100 font-bold text-slate-800 text-[10px] uppercase tracking-[6px] bg-white text-center opacity-30">Pending Financial Settlements</div>
...
const JobsTable = ({ jobs, title }) => (
  <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden animate-in fade-in duration-300">
    <div className="px-10 py-6 border-b font-bold uppercase text-[10px] tracking-[8px] bg-white text-slate-800 flex justify-between items-center opacity-40"><span>{title}</span><div className="px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-full font-bold italic tracking-tighter text-[10px]">{jobs?.length || 0} TOTAL OPS</div></div>
        ...
        {(!jobs || jobs.length === 0) ? (
          ...
          jobs.map(j => (
            <tr key={j._id} className="hover:bg-slate-50/50 transition-colors group">
              ...
              <td className="px-10 py-8"><span className={`px-6 py-2.5 rounded-full text-[9px] font-bold uppercase border-2 transition-all shadow-lg ${j.status === 'done' ? 'bg-emerald-50 border-emerald-100 text-emerald-600 shadow-emerald-50' : 'bg-indigo-50 border-indigo-100 text-indigo-600 shadow-indigo-50'}`}>{j.status.replace('_',' ')}</span></td>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 animate-in fade-in duration-300">
      <div className="bg-[#020617] p-12 rounded-[50px] shadow-2xl h-fit border border-white/5 relative overflow-hidden text-center">
        ...
        <h3 className="font-bold mb-12 uppercase text-[9px] tracking-[6px] text-indigo-500">Initialize Core Module</h3>
        <div className="space-y-8">
